...
class DeputiesLawProjectsTextTestCase(TestCase):
    def test_get_text_from_infobase_doesnt_overwrite_link_if_data_obtained_is_empty(
        ...
        with mock_method(Session, "post", return_value=FakeResponse(200, pdf_link)):
            with mock_method(DeputiesLawProyectsText, "_get_pdf_text", return_value=""):
                with mock_method(
                    DeputiesLawProyectsText, "_get_text_from_infobase", return_value=""
                ):
                    text, link = DeputiesLawProyectsText._get_text("2528", "D", "2023")
                    self.assertEqual(
                        link,
...
class SenateLawsProjectTextTestCase(TestCase):
    def test_parses_correctly_number_and_year(self):
        number, source, year = SenateLawProyectsText._parse_input("0003", "s", "2023")
    ...
    def test_initial_text_is_retrieved_correctly_if_final_isnt_loaded(self):
        text, link = SenateLawProyectsText._get_text("1047", "S", "2004")
